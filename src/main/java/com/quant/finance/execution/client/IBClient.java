...
public class IBClient implements EWrapper {
  ...
  private final AtomicInteger orderIds = new AtomicInteger(ThreadLocalRandom.current().nextInt());
  private static IBClient ibClient = new IBClient();
  ...
  public static synchronized IBClient getInstance() {
    if (ibClient == null) {
  ...
  public void orderStatus(int orderId, String status, Decimal filled, Decimal remaining,
                          double avgFillPrice, long l,
                          int i1, double v1, int i2, String s1, double v2) {
    log.info("ORDER STATUS: orderId={} status={} filled={} remaining={} avgPrice={}", orderId,
        status, filled.value(), remaining.value(), avgFillPrice);
  }
  ...
  public void error(int i, long l, int i1, String s, String s1) {
    log.error("ERROR. orderId: {}, code: {}, msg: {}", i, i1, s);
  }
